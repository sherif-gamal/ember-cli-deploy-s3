...
module.exports = CoreObject.extend({
  ...
  _putObjects: function(filePaths, options) {
    ...
    return filePaths.map(function(filePath) {
      ...
      // Drop app/index.html to fix cloudFront caching issue, it will be uploaded later using deploy-s3-index
      if (key.indexOf('app/index.html') !== -1) {
        plugin.log('app/index dropped');
        return;
      }
      // Move static-root files to root
      if (key.indexOf('static-root') !== -1) {
        key = key.substring(key.indexOf('static-root') + 12);
      }
